...
# 
# ### current version:
# - layout of LSS components (`Hub_Rotor_LSS_Frame`) only
# 
# ### TODO:
# - optimization on DVs `L_h1, L_12, lss_diameter, lss_wall_thickness` with constraints on stresses, deflections, angles
# 
# references
...
results_dir = "results_m4w_layout"
...
if opt_flag:
    ...
    # prob.model.add_design_var("bedplate_web_thickness", lower=4e-3, upper=5e-1, ref=1e-2)     #TODO: add next
    # prob.model.add_design_var("bedplate_flange_thickness", lower=4e-3, upper=5e-1, ref=1e-2)  #TODO: add next
    # prob.model.add_design_var("bedplate_flange_width", lower=0.1, upper=2.0)                  #TODO: add next
    ...
    # prob.model.add_constraint("constr_bedplate_vonmises", upper=1.0)  #TODO: add next
    ...
    # prob.model.add_constraint("constr_stator_deflection", upper=1.0)  #TODO: add next
    # prob.model.add_constraint("constr_stator_angle", upper=1.0)       #TODO: add next
    
    # 3. hub dia to accom. blades' roots
    #prob.model.add_constraint("constr_hub_diameter", lower=0.0)
    
    # 4. target overhang and hub height
    # prob.model.add_constraint("constr_length", lower=0.0)             #TODO: add later
    # prob.model.add_constraint("constr_height", lower=0.0)             #TODO: add later
...
if flag_DOE: #NOTE: running 72 mins! on my PC for 6 levels x 4 DVs (khayr insha'Allah)
    ...
    # Add objective
    prob.model.add_objective("nacelle_mass", ref=1e6)
